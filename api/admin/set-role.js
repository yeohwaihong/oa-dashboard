...
export default async function handler(req, res) {
  try {
    ...
    if (roleRaw === null || roleRaw === "") {
      const deleted = await serviceClient.from("user_roles").delete().eq("user_id", userId);
      if (deleted.error) return sendJson(res, 500, { error: deleted.error.message || "Failed to remove role." });
      return sendJson(res, 200, { ok: true });
    }

    const upserted = await serviceClient.from("user_roles").upsert({ user_id: userId, role: roleRaw }, { onConflict: "user_id" });
    if (upserted.error) return sendJson(res, 500, { error: upserted.error.message || "Failed to set role." });
